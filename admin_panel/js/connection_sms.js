var grid_sms;
var sms_connection = {
    controller_url: 'controller/controller_sms.php'
    , debug_mode: false
...
sms_connection.set = function (number, text, deviceid, reqnumber) {
    var nowDate = DateTime.getTimeNowTimeStamp();
    var createBy = '0';
    var param = {
        act: "sms_set",
        number: number,
...
sms_connection.get_grid_call_back = function (data) {
    ...
    grid_sms.cols = [
        ...
        {name: 'smsId', hidden: true, type: 'text'},
        {
            name: 'registerDate',
            hidden: false,
            type: 'text'
    ...
    grid_sms.RightToLeft = true;

    grid_sms.actions = [{name: 'show', ClassName: 'glyphicon glyphicon-eye-open', attribute: [{name: 'onclick' , value: 'showMsg(this)'}]}];
...
function showMsg(elem){
    var sms = grid_sms.get_field_of_row(elem , 'text');
    document.getElementById('DetailBody').innerHTML = sms;
